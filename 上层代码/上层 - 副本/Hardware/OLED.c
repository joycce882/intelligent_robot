...
void OLED_Init(void)
{
  ...
  OLED_WriteCommand(0xAE); //关闭显示（0xAE关闭OLED，0xAF打开OLED）
  OLED_WriteCommand(0x20);  //设置GDDRAM寻址模式，由下面命令决定  
  OLED_WriteCommand(0x10);  //00：水平寻址模式; 01：垂直寻址模式; 10：页寻址模式;
  OLED_WriteCommand(0xb0);  //设置页寻址模式的页起始位置
  OLED_WriteCommand(0xc8);  //设置扫描方向
  OLED_WriteCommand(0x00); //设置列地址起始地址低位
  OLED_WriteCommand(0x10); //设置列地址起始地址高位
  OLED_WriteCommand(0x40); //设置GDDRAM起始行
  OLED_WriteCommand(0x81); //设置对比度
  OLED_WriteCommand(0xff); //亮度调节 0x00~0xff
  OLED_WriteCommand(0xa1); //左右反置关
  OLED_WriteCommand(0xa6); //正常显示模式
  OLED_WriteCommand(0xa8); //设置复用率
  OLED_WriteCommand(0x3F); 
  OLED_WriteCommand(0xa4); //输出GDDRAM中的显示数据
  ...
  OLED_WriteCommand(0xdb); //设置Vcomh输出
