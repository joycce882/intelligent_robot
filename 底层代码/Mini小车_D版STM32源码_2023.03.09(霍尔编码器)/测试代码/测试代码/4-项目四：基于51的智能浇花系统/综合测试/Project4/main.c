...
sbit LCD1602_RS = P3^6;	 //RS端
sbit LCD1602_RW = P3^5;	 //RW端
sbit LCD1602_EN = P3^4;	 //EN端

//ADC0832
sbit cs  = P2^4;//片选使能，低电平有效
sbit clk = P2^3;//芯片时钟输入
sbit dio = P2^2;//数据输入DI与输出DO
...
void main()
{
	...
	while(1)
	{
		...
		humidity = adc * 500.0 / 255;//将adc值换算成湿度，公式需修改
		if(humidity < 55) RELAY_Control(0);		 //当湿度，，控制继电器，，
		LCD1602_Dis_Str(0, 0, &LCD_buffer[0]);	//显示字符串
		LCD1602_Dis_Str(0, 1, &humidity);				//显示字符串
	}
}
