...
{ 
    ...
    {	
			...
			vTaskDelayUntil(&lastWakeTime, F2T(100)); //这里的100是RATE_100_HZ，在SYStem.h文件里面定义。
			...
			if(Check==0) //If self-check mode is not enabled //如果没有启动自检模式
			{
				...
				if(command_lost_count>100/*RATE_100_HZ*/ && APP_ON_Flag==0 && Remote_ON_Flag==0 && PS2_ON_Flag==0) //不是APP、PS2、航模遥控模式，就是CAN、串口1、串口3控制模式
...
{	
	u8 tmp;
	tmp=click_N_Double_MPU6050(50); //下面一行在MPU6050写完之后再写
